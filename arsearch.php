...
if (isset($_GET['q'])) {
  $q = $_GET['q'];
  $statement = $con->prepare("select * from artisti where nume like :nume or prenume like :prenume");
  $statement->execute([
    ':nume' => '%' . $q .'%',
    ':prenume' => '%' . $q .'%'
  ]);
} else {
  $statement = $con->prepare('select * from artisti');
  $statement->execute();
}
$artisti = $statement->fetchAll(PDO::FETCH_OBJ);
 ?>
<!doctype html>
<html lang="en">
  <head>
